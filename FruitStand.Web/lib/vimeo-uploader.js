    ...
    xhr.onload = function (e) {
        ...
        if (e.target.status < 400) {
            ...
            if (this.videoUrl != null) {
                //this.sendFileViaURL_();

                $.post("http://localhost:4835/api/vimeo/UploadVideo", {
                    "token": this.token,
                    "url": this.url
                }, function (response) {
                    if (response.success) {
                        var data = JSON.parse(response.extra);
                        //  uploadFileonVimeo(data.access_token);
                    }
...
MediaUploader.prototype.sendFileViaURL_ = function () {
    var content = this.file;
    var end = this.file.size;
